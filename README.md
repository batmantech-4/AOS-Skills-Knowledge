# AOS-Skills-Knowledge
LifeOS-The Life Operating System — an intent engineering platform that moves you from your current state to your ideal state, in life and work.
https://github.com/danielmiessler/LifeOS 
Unified Efficient Fine-Tuning of 100+ LLMs & VLMs 
https://github.com/hiyouga/LlamaFactory
Ingest, parse, and optimize any data format
https://github.com/adithya-s-k/omniparse
Orca is the ADE for working with a fleet of parallel agents.
https://github.com/stablyai/orca
100+ AI Agents, Agent Skills and RAG Apps - Free and Open Source.
https://github.com/Shubhamsaboo/awesome-llm-apps
Youtube automation  agent
https://github.com/darkzOGx/youtube-automation-agent
The fastest browser for AI agents to run browser automation
https://github.com/citrolabs/ego-lite
Edit videos with coding agents
https://github.com/browser-use/video-use
book-to-skill, an open-source tool that converts any technical book (or a whole folder of docs) into a structured skill your coding agent can pull up on command.
https://github.com/virgiliojr94/book-to-skill
VibeVoice is a free open-source voice AI that can generate 90-minute multi-speaker audio
https://github.com/microsoft/VibeVoice
The design language that makes your AI harness better at design.
https://github.com/pbakaus/impeccable 
A free, open source, and extensible speech-to-text application that works completely offline.
https://github.com/cjpais/handy 
"OpenSpace: The Skill Management Layer for AI Agents" 
https://github.com/HKUDS/OpenSpace 
Download Udemy and Hotmart courses, YouTube videos, music and books
https://github.com/tonhowtf/omniget 
Cognee gives AI agents persistent long-term memory across sessions.
https://github.com/topoteretes/cognee 
wigolo gives your AI agent live web search, fetch, and research — all from your own computer with no API keys or cloud. 
https://github.com/KnockOutEZ/wigolo 
Battle-tested Claude Code, Claude Desktop, OpenAI Codex, and Cursor setup with skills, commands, hooks, subagents, and MCP servers, plus Kimi, MiniMax, and GLM API support.
https://github.com/fcakyon/claude-codex-settings 
Practical patterns, starters & CLI tools for loop engineering with AI coding agents. Design systems that prompt and orchestrate agents (inspired by Addy Osmani and Boris Cherny). Includes loop-audit, loop-init, loop-cost.
https://github.com/cobusgreyling/loop-engineering 
ComfyUI is the AI creation engine for visual professionals who demand control over every model, every parameter, and every output. 
https://github.com/comfy-org/comfyui 
first open-source, agentic video production system. 
https://github.com/calesthio/OpenMontage 
Jan is an open source alternative to ChatGPT that runs 100% offline on your computer.
https://github.com/janhq/jan 
The fastest browser for AI agents to run browser automation, built for sharing your logged-in browser state with your AI agents, like Codex or Claude Code, without disturbing you. Zero cost, zero config.
https://github.com/citrolabs/ego-lite 
The definitive, install-ready index mapping out over 82 built-in skills, a massive optional catalog, and community-vetted plugins.
https://github.com/ZeroPointRepo/awesome-hermes-skills
A comprehensive directory tracking optional plugins, memory providers (like Honcho), specialized execution surfaces, and multi-agent bridges.
https://github.com/0xNyk/awesome-hermes-agent
 Curated, real-world deployment maps demonstrating multi-agent orchestration, daily automated briefings, and cross-functional team dynamics.
 https://github.com/aliaihub/awesome-hermes-usecases
 A specialized plugin/repository that enables an autonomous Hermes agent to monitor your ongoing workflows and write its own SKILL.md repository files completely on autopilot.
 https://medium.com/coding-nexus/5-hermes-plugins-that-turn-an-ai-agent-into-a-self-improving-system-a0bb75a12a4e






